...
if __name__ == "__main__":
    ...
    def _print_enums(enums, format, linefmt="\t%s = %i,"):
        ret = []
        # linefmt = "\t%s = %i,"
    ...
    def _print_enums_seperate(enums, format, linefmt="\t%s = %i,"):
        ret = []
        for enum in sorted(enums):
            sorted_pairs = sorted(enums[enum].items(), key=lambda k: k[1])
            lines = "\n".join(linefmt % (name, value) for name, value in sorted_pairs)
    ...
    if format == "--ts":
        ...
        _print_enums_seperate(enums, "enum %s {\n\toption allow_alias = true;\n%s\n}", linefmt="\t%s=%i;")
    else:
